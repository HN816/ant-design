 ...
 */
export const usePopupScope = (options = {}) => {
  const {
  ...
  const removePortalTimerRef = React.useRef(null);
  const checkAndCloseTimerRef = React.useRef(null);
  ...
  const mergedGetPopupContainer = React.useCallback(triggerNode => {
  ...
  const wrappedPopupRender = React.useMemo(() => {
    ...
    return (...args) => (/*#__PURE__*/React.createElement(ConfigProvider, {
  ...
  const handlePopupVisibleChange = React.useCallback(open => {
    if (open === false) {
      cancelAllTimers();
